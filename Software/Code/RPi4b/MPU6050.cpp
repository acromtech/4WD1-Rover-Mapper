...
MPU6050::MPU6050(int8_t addr, bool run_update_thread) {
	...
	if (f_dev < 0) std::cout << "ERR (MPU6050.cpp:MPU6050()): Failed to open /dev/i2c-1. Please check that I2C is enabled with raspi-config\n";  //Catch errors//Print error message
	status = ioctl(f_dev, I2C_SLAVE, MPU6050_addr); //Set the I2C bus to use the correct address
	if (status < 0) std::cout << "ERR (MPU6050.cpp:MPU6050()): Could not get I2C bus with " << addr << " address. Please confirm that this address is correct\n"; //Print error message
	...
	if (run_update_thread) std::thread(&MPU6050::updateAllAngle, this).detach(); //Create a seperate thread, for the update routine to run in the background, and detach it, allowing the program to continue
	calc_yaw = true;
}
...
float MPU6050::getRollAngle() 	{ return _angle[0]; }
float MPU6050::getPitchAngle() 	{ return _angle[1]; }
float MPU6050::getYawAngle() 	{ return _angle[2]; }

void MPU6050::updateAllAngle() {
